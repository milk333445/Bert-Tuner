...
class InferenceModel:
    ...
    def predict(self, texts, threshold=0.5):
        ...
        with torch.no_grad():  
            ...
            max_prob, max_indices = torch.max(outputs, dim=1, keepdim=True)
            max_indices[torch.where(max_prob <= threshold)[0]] = 99
            predictions = max_indices.cpu().numpy().squeeze()  
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", default='analysis_content_clean.csv', type=str,
                        help="Path to the CSV file containing the texts to predict.")
    parser.add_argument("--output_path", type=str, default='analysis_content_clean_preds.csv',
                        help="Path where the predictions will be saved.")
    parser.add_argument('--threshold', type=float, default=0.7, help='Threshold for prediction.')
    
    parser.add_argument('--max_length', type=int, default=128, help='Max length of the input text.')
    
    parser.add_argument('--batch_size', type=int, default=1000, help='batch_inference for inferencer.')
